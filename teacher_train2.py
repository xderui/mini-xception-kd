...
# model.load_state_dict(torch.load('weights/resnet101_final1.pth', map_location=device))
model.to(device)

# 数据加载
from rafdb_face import RAFDBFACE
# train_dataset = FER2013("train", input_size=input_size)
# test_dataset = FER2013("test", input_size=input_size)
train_dataset=RAFDBFACE(mode='train')
test_dataset=RAFDBFACE(mode='test')
...
alpha = torch.Tensor([(weights_sum-i)/(9*weights_sum)*10 for i in weights])
print(alpha)
loss_fn = FocalLoss(class_num=7,alpha=alpha)
